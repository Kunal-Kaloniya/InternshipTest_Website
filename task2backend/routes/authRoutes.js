...
router.post("/signup", async (req, res) => {
    ...
    if (!username || !email || !password) return res.status(401).json({message: "All credentials are required!"});
...
router.post("/login", async (req, res) => {
    ...
    if (!email || !password) return res.status(401).json({message: "All credentials are required!"});
    if (!validator.isEmail(email)) return res.status(400).json({message: "Invalid email format"});
